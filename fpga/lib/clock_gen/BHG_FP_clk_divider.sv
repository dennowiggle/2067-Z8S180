...
// WTM : Yosys can't print floats so remove the sign to prevent -ve number errors.
// localparam signed clk_dif_hzX100   = (clk_tru_hzX100 - (OUTPUT_CLK_HZ*100))                            ;
localparam signed clk_dif_hzX100   = (clk_tru_hzX100 > (OUTPUT_CLK_HZ*100))?
                                        (clk_tru_hzX100 - (OUTPUT_CLK_HZ*100)) :
                                        ((OUTPUT_CLK_HZ*100) - clk_tru_hzX100);
localparam signed clk_ppm_x100     =  clk_dif_hzX100 * 1e6 / OUTPUT_CLK_HZ                             ; // Calculate the PPM offset error X 100.
...
localparam real   f_dif            = clk_dif_hzX100;
...
$display("  XXX   --------------------------------------------- (Floats only accurate to 2 decimal places)"); 
//**** %0.2f works in Modelsim, not Quartus. ****
//**** %f does not work with yosys.          ****
...
$display("  XXX   True output freq    = %0d.%0d Hz",        $rtoi(f_tru/100), $rtoi( f_tru % 100 ) );
$display("  XXX   Frequency error     = %0d.%0d Hz",        $rtoi(f_dif/100), $rtoi( f_dif % 100 ) );
$display("  XXX   Frequency error     = %0d.%0d ppm",       $rtoi(f_ppm/100), $rtoi( f_ppm % 100 ) );
$display("  XXX   Periodic jitter     = +/- %0d.%0d ns",   $rtoi(f_jit/100), $rtoi( f_jit % 100 ) );
$display("  XXX   ---------------------------------------------");
...
$display("  XXX   ~Divider to 6dp     = %0d.%0d"                                                             , clk_per_int, $rtoi(1e6*f_div/65536));
$display("  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
$display("");
end
endgenerate
